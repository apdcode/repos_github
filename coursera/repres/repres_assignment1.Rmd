...
#   2     Handle missing observations for steps
missingSteps <- nrow(subset(dx1, !is.na(dx1$steps)))

#   2.1   Find average steps per day for each interval
dx3 <- subset(dx1, !is.na(dx1$steps))
dt_avgsteps <- data.table(sqldf("SELECT interval, avg(steps) as steps 
        FROM dt1
        Group by interval"))

#   2.2   Replace missing values in dx4 with values in dt_avgsteps
dt_missing <- dx1
...
A total of `r nmiss` missing observations for steps in the original dataset have been replaced with the average number of steps for the corresponding category. The sum of steps added to the original dataset is `r fakesteps`
